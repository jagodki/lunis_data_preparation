DO $$
	DECLARE
        ...
        line_id CHARACTER VARYING := 'edge_id';
	...
	BEGIN
		...
		FOR i IN SELECT line_id FROM roads.roads_rdbl LOOP
            ...
            FOR j IN SELECT * FROM roads.road_nodes_rdbl WHERE old_id = i.line_id ORDER BY number_on_line ASC LOOP
				...
				IF equidistance IS NOT NULL THEN
                	UPDATE roads.road_nodes_rdbl SET neighbours = neighbours || '{{predecessor::CHARACTER VARYING, equidistance}}' WHERE point_id = j.point_id;
                END IF;
